...
contract CroToken is ReleasableToken, MintableToken, UpgradeableToken {
    ...
    public UpgradeableToken(msg.sender) {
        ...
        if (_initialSupply > 0) {
            ...
            mint(secondarySaleReserveWallet, thirtyPerCent); //30

            mint(mainNetLaunchIncentiveReserveWallet, twentyPerCent);//20

            mint(capitalReserveWallet, twentyPerCent);//20 

            mint(ecosystemGrantsReserveWallet, twentyPerCent); //20 

            mint(airdropReserveWallet, tenPerCent); //10
