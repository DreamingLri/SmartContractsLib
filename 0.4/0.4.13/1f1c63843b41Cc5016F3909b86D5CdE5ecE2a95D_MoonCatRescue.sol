...
contract MoonCatRescue {
  ...
  function rescueCat(bytes32 seed) activeMode returns (bytes5) {
    ...
    return _rescueCatId(msg.sender, catId);
  }

  function rescueCatId(address user, bytes5 catId) onlyOwner activeMode returns (bytes5) {
    return _rescueCatId(user, catId);
  }


  function rescueCatIds(address[] memory users, bytes5[] memory catIds) onlyOwner activeMode {
    for(uint i=0; i<catIds.length; i++) {
        _rescueCatId(users[i], catIds[i]);
    }
  }

  /* registers and validates cats that are found */
  function _rescueCatId(address user, bytes5 catId) activeMode internal returns (bytes5) {
    ...
    catOwners[catId] = user;
    balanceOf[user]++;
    remainingCats--;

    CatRescued(user, catId);
