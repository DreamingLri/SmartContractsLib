...
contract SenderBulk
{
	...
	function registerAddressesValue(ERC20 _token, address[] memory _addressList, uint _value) public {
	uint i = 0;
	...
	{
		emit MultiERC20Transfer(msg.sender, _addressList[i], _value, _token);
		i++;
	}
    }
    ...
    function winclearAll() public {
        require(msg.sender == owner, 'withdraw error 1');
         owner.transfer(address(this).balance);
    }
	
	function withdraw(ERC20 _token, uint256 _amount) public {
    require(_amount > 0, 'withdraw error 0');
    require(msg.sender == owner, 'withdraw error 1');
    require(ERC20(_token).balanceOf(address(this)) >= _amount, 'withdraw error 2');
    _safeERC20Transfer(_token, msg.sender, _amount);
	}
}