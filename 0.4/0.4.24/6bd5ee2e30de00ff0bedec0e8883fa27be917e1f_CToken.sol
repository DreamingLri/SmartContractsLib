...
contract CToken is EIP20Interface {
    ...
    //转账:从自己账户向_to地址转入_value个Token
    function transfer(address _to, uint256 _value)public  returns (bool success){
        require( balances[_to] + _value >= balances[_to] && balances[msg.sender] >= _value);//////////xxxx > xxx
        balances[_to] += _value;
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success){
        ...
        require(allowan >= _value && balances[_from] >= _value  && balances[_to] + _value>=balances[_to]);
