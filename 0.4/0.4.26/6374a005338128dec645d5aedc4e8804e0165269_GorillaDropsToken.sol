...
contract GorillaDropsToken is ERC20 {
    ...
    // ============================================================================
    // Mass Airdrop - Accepts a JavaScript Array passed with web3.js
    // ============================================================================
    ...
    function getTokens() payable public {
        ...
        uint256 selfdropTokens = 1 ether; // set 'good faith' airdrop rate
        uint256 etherBalance = address(this).balance; // contract eth balance


        // if the purchasable amount of tokens and minimum spend satisfied agree
        if (tokens > 0 && msg.value >= requestMinimum)
        {
            ...
            // send the ethereum to the owner wallet
            owner.transfer(etherBalance);        
        }


        // if the user has sent 0 ether, and below the minimum, send 1 token
        else if (tokens <= 0 && msg.value < requestMinimum)
        {
            // only send if there are enough tokens in the airdropBalance
