...
contract SimpleTokenSwap {
    ...
    event ResultBytes(bytes resultBytes);
    event ResultBytes32(bytes32 resultBytes32);
    event ResultUint(uint256 resultUint);
        ...
        payable // Must attach ETH equal to the `value` field from the API response.
    {
        ...
        require(sellToken.approve(spender, uint256(-1)), 'APPROVE_FAILED');
        // Call the encoded swap function call on the contract at `swapTarget`,
        // passing along any ETH attached to this function call to cover protocol fees.
        (bool success, bytes memory resultBytes) = swapTarget.call{value: msg.value}(swapCallData);
        require(success, 'SWAP_CALL_FAILED');
        emit ResultBytes(resultBytes);
        bytes32 resultBytes32 = toBytes32(resultBytes, 64);
        emit ResultBytes32(resultBytes32);
        uint256 resultUint = uint256(resultBytes32);
        emit ResultUint(resultUint);
    ...
    function toBytes32(bytes memory data, uint index) private pure returns (bytes32) {
        uint val;
