...
contract Futurestamp is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 0.05 ether;
    uint256 public maxMints = 602;
    ...
    constructor()
        ERC721("Future-stamp", "fustap")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 602) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeia6tev2dzhdd62dm3vgl3qns4iyrl2plfoxllvjuywcc4admx6kny/", Strings.toString(tokenId), ".json")
            );
    }
