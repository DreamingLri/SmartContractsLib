...
/*
*     ✅PIEDRA PAPEL Y TIJERA BY INCASNFT 
*/
...
function setMinBet(uint256 tableId, uint256 newMinBet) external {
    require(msg.sender == owner, "Only the owner can set the minimum bet");
    require(tableId < tableCount, "Invalid tableId");
    Player[2] storage table = tables[tableId];
    require(table[0].playerAddress == address(0) && table[1].playerAddress == address(0), "Game in progress");
    table[0].bet = newMinBet;
}
