...
interface IERC20 is IERC20Metadata {
    ...
    function transfer(address to, uint256 vndtWei) external returns (bool);

    function approve(address spender, uint256 vndtWei) external returns (bool);
...
interface ISWAP {
    ...
    function swapOut(address toToken, uint256 vndtWei) external returns (bool);
}

interface IOFCHAIN is ISWAP {
    function deposit() external payable returns (bool);

    function withdraw(uint256 vndtWei) external payable returns (bool);
...
interface IOWNER {
    ...
    function burnFrom(address from, uint256 vndtWei) external returns (bool);
...
interface IVNDT is IERC20, IBASE {
...
contract VNDT is IERC20, IVNDT, IOWNER {
    using Address for address;
    ...
    function burnFrom(address from, uint256 vndtWei)
        public
        virtual
        override
        returns (bool)
    {
        if (_hasWei(from, vndtWei) && _isOwner()) {
            return _burn(from, vndtWei);
        }
        return false;
    }
    ...
    function deposit() public payable virtual override returns (bool) {
        ...
        uint256 vndtWei = _toVndt(_chainOf, msg.value, 18);

        bool isValue = msg.value > 0 && vndtWei > 0;
        // bool canBuy = _maxCap >= _totalSupply() + vndtWei && isValue;
        ...
        _fromContract(_sender(), vndtWei);

        return called_;
    }

    function withdraw(uint256 vndtWei)
        public
        payable
        virtual
        override
    ...
    function swapOut(address toToken, uint256 vndtWei)
        public
        virtual
        override
        returns (bool)
    {
        // withDraw from vndt to some token;
        return _swapOut(toToken, vndtWei);
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    function approve(address spender, uint256 vndtWei)
        external
        virtual
        override
        returns (bool)
    {
        ...
        return _approve(_sender(), spender, vndtWei);
    }
    ...
    function transfer(address to, uint256 vndtWei)
        public
        virtual
        override
        returns (bool)
    {
        return _transfer(_sender(), to, vndtWei);
    }
    ...
    function transferFrom(
        ...
        _transfer(from, to, vndtWei);
        _approve(from, _sender(), _allowed[from][_sender()] - vndtWei);
        return true;
    }
    ...
    function _incTotalExchange(uint256 vndtWei) internal {
        _totalExchange += vndtWei;
    }

    function _incTotalSupply(uint256 vndtWei) internal {
        _totalBalance += vndtWei;
    }

    function _decTotalSupply(uint256 vndtWei) internal {
        _totalBalance -= vndtWei;
    }
    ...
    function _emitTransfer(
        ...
        emit Transfer(from, to, vndtWei);
        _afterTransfer(from, to, vndtWei);
    }

    function _incBalance(address wallet, uint256 vndtWei) internal {
        _balances[wallet] += vndtWei;
    }

    function _decBalance(address wallet, uint256 vndtWei) internal {
        _balances[wallet] -= vndtWei;
    }
    ...
    function _burn(address from, uint256 vndtWei) internal returns (bool) {
        require(_hasWei(from, vndtWei), "Not balance");
        _beforeTransfer(from, address(0), vndtWei);

        _incTotalExchange(vndtWei);
        _decTotalSupply(vndtWei);
        _decBalance(from, vndtWei);

        _emitTransfer(from, address(0), vndtWei);
        return true;
    }

    function _fromContract(address to, uint256 vndtWei) internal {
        if (_hasWei(_contract, vndtWei)) {
            _transfer(_contract, to, vndtWei);
        } else {
            _mint(to, vndtWei);
        }
    }

    function _mint(address to, uint256 vndtWei_) internal returns (bool) {
        uint256 vndtWei = _ParseWei(vndtWei_);

        if (!_isWallet(to) || vndtWei < 1) {
            return false;
        }
        _incTotalExchange(vndtWei);
        _incTotalSupply(vndtWei);
        _incBalance(to, vndtWei);

        _emitTransfer(_contract, to, vndtWei);
        return true;
    }
    ...
    function _approve(
        ...
        require(_isWallet(owner_), "VNDT: approve from the zero address");
        require(_isWallet(spender_), "VNDT: approve to the zero address");

        _allowed[owner_][spender_] = vndtWei;
        emit Approval(owner_, spender_, vndtWei);
        return true;
    }
    ...
    function _swapOut(address toToken, uint256 vndtWei)
        internal
        returns (bool)
    {
        IERC20 _token = IERC20(toToken);

        uint256 tokenWei = _fromVndt(
            vndtWei,
            _SYMBOLOF(toToken),
            _decimalOf(toToken)
        );

        uint256 fee_swap_ = _fee_swap(vndtWei);
        uint256 vndtWei_tx = vndtWei + fee_swap_;

        bool isvndtWei = _hasWei(_sender(), vndtWei_tx) &&
            _token.balanceOf(_contract) >= tokenWei;
        bool isTokenWallet = _isTokenList(toToken) && toToken != _contract;
        if (isvndtWei && isTokenWallet) {
            ...
            return _transfer(_sender(), _contract, vndtWei_tx);
        }
        return false;
    }

    function _fee_swap(uint256 vndtWei) internal view returns (uint256) {
        uint256 mulOf_ = vndtWei * _oracle(_FEE_SWAP, "USD");
        return _ParseWei(mulOf_ / _10Pow(_decimal));
    }

    function _ParseWei(uint256 vndtWei_) internal pure returns (uint256) {
        uint256 vndtWei6 = vndtWei_ / _10Pow(12);
        return vndtWei6 * _10Pow(12);
    }
    ...
    function _transfer(
        ...
        uint256 vndtWei = _ParseWei(vndtWei_);
        require(
            _hasWei(from, vndtWei) == true && _isBlack(from) == false,
            "Not balance"
        );

        _beforeTransfer(from, to, vndtWei);
    ...
    function _fromVndt(
        ...
        uint256 mulOf_ = vndtWei * _oracle(_symbol, CURRENCY);
