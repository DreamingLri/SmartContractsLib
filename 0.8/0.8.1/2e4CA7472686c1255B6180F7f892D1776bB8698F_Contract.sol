...
contract Contract is ERC721, Ownable {
    ...
    constructor() ERC721("TGR TEST", "TGRTEST") {
        for(uint256 i = 1; i <= RESERVED_TOKENS; i++){
            _safeMint(RESERVED_TOKENS_ADDRESS, i);
        }
    }
    ...
    //mapping(address => uint256) payoutBalances;
    // function payout(uint256 payoutAmount) external onlyOwner {
    //     uint256 totalTokens = soldAmount + RESERVED_TOKENS;
    //     require(payoutAmount * totalTokens <= address(this).balance, "Not enough balance in contract");
    ...
    //         require(payable(owner).send(payoutAmount));
    //     }
    // }
    ...
    function mint(uint256 _amount, address _recipient) external payable {
        ...
        for(uint256 i = 0; i < _amount; i++){
            _safeMint(_recipient, soldAmount + RESERVED_TOKENS + i + 1);
        }
