...
/**
 *Submitted for verification at Polygon on 2023-06-11
*/
...
contract PresaleContract is ReentrancyGuard, Ownable, Pausable {
    ...
    struct SalePara {
        ...
        //usdt Minimum Purchase Amount
        uint256 usdt_minimum_amount;
    }

    IERC20 public usdt_interface;
    IERC20 public weth_interface;
    IERC20 public token_interface;
    AggregatorV3Interface internal eth_price_feed;
    AggregatorV3Interface internal matic_price_feed;

    mapping(address => User) public users_list;
    SalePara public salePara;
    ...
    //  _pay_method = ( 1:matic, 2:eth_card, 3:usdt )

    constructor(
        address token_,
        address usdt_address_,
        ...
        uint256 usdt_minimum_amount_
    ) {
        usdt_interface = IERC20(usdt_address_);
        weth_interface = IERC20(weth_address_);
        token_interface = IERC20(token_);
        eth_price_feed = AggregatorV3Interface(eth_oracle);
        matic_price_feed = AggregatorV3Interface(matic_oracle);

        presale_ended = false;

        salePara = SalePara(wallet_, usdt_to_token_rate_, 0, usdt_round_cap_ * (10**6), usdt_minimum_amount_ * (10**6));
    }
    ...
    function get_eth_in_usdt() internal view returns (uint256) {
        (, int256 price, , , ) = eth_price_feed.latestRoundData();
        price = price * 1e10;
        return uint256(price);
    }

    function get_matic_in_usdt() internal view returns (uint256) {
        (, int256 price, , , ) = matic_price_feed.latestRoundData();
        price = price * 1e10;
        return uint256(price);
    }
        ...
        returns (bool)
    {
        ...
        require(amount_in_usdt >= salePara.usdt_minimum_amount,"BUY ERROR : Minimum purchase amount required.");
        require(
            salePara.usdt_round_raised + amount_in_usdt <
                salePara.usdt_round_cap,
        ...
        uint256 amount_in_tokens = (amount_in_usdt *
            salePara.usdt_to_token_rate) * 1e12;
        ...
        returns (bool)
    {
        ...
        (bool sent,) = salePara.wallet.call{value: msg.value}("");
    ...
    function buy_with_weth(uint256 amount_)
        external
        nonReentrant
        whenNotPaused
        canPurchase(_msgSender(), amount_)
        returns (bool)
    {
        uint256 amount_in_usdt = (amount_ * get_eth_in_usdt()) / 1e30;
        require(amount_in_usdt >= salePara.usdt_minimum_amount,"BUY ERROR : Minimum purchase amount required.");
        require(
            salePara.usdt_round_raised + amount_in_usdt <
                salePara.usdt_round_cap,
        ...
        uint256 allowance = weth_interface.allowance(msg.sender, address(this));

        require(amount_ <= allowance, "BUY ERROR: Allowance is too small!");

        (bool success_receive, ) = address(weth_interface).call(
            abi.encodeWithSignature(
                "transferFrom(address,address,uint256)",
                msg.sender,
                salePara.wallet,
        ...
        emit Deposit(_msgSender(), 2, amount_in_usdt, amount_in_tokens);
    ...
    function get_current_raised() external view returns (uint256) {
        return salePara.usdt_round_raised;
    }

    function set_usdt_minimum_amount(uint256 amount) external onlyOwner {
        salePara.usdt_minimum_amount = amount * (10**6);
    }
    ...
    function withdrawToken(address tokenContract, uint256 amount, address toaddress) external onlyOwner {
        IERC20(tokenContract).transfer(toaddress, amount);
    }
