...
contract PolyKingWhale is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PKWhaleDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("PKWhale", "PKWhale") {
    	...
    	dividendTracker = new PKWhaleDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PKWhale: The dividend tracker already has that address");

        PKWhaleDividendTracker newDividendTracker = PKWhaleDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PKWhale: The new dividend tracker must be owned by the PKWhale token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PKWhale: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PKWhale: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PKWhale: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PKWhale: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PKWhale: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PKWhale: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PKWhale: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "PKWhale: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "PKWhale: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "PKWhale: The fixed-sale is only available to certain participants at the start");
            }
...
contract PKWhaleDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("PKWhale_Dividend_Tracker", "PKWhale_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10 * 10**9 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "PKWhale_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "PKWhale_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PKWhale contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "PKWhale_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PKWhale_Dividend_Tracker: Cannot update claimWait to same value");
